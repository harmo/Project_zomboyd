...
Optimized for maps that do not make heavy use of tile properties.  If I find
that it is used a lot then I can rework it for better performance.
...
a sample map and viewer.  The viewer is based of my other library 'lib2d', and
...
def load_tmx(filename):
    ...
    def parse_layer(tilesets, node):
        ...
        for y in xrange(layer.height):
            ...
            # since we will never use enough tiles to fill a 32-bit int
            # store as 16-bit ints
...
def load_pygame(filename):
    ...
    for firstgid, t in sorted([ (t.firstgid, t) for t in tiledmap.tilesets ]):
        ...
        for (y, x) in product(y_range, x_range):
            ...
            # somewhat handle transparency. colorkey is not tested
            if t.trans == None:
